...
export default function AdminDashboard() {
    ...
    const [products, setProducts] = useState([]); // New Product State
    ...
    const [productFormData, setProductFormData] = useState({
        name: "", category: "", price: "", description: "", imageUrl: "", inStock: true
    });
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (activeTab === "pets" && user.role === "ADMIN") {
                ...
                const res = await fetch(`/api/applications?page=${page}&limit=10&sort=newest`, { headers });
                if (res.ok) {
                    const data = await res.json();
                    setApplications(data.applications);
                    setTotalPages(data.pagination.totalPages);
                }
            } else if (activeTab === "products" && user.role === "ADMIN") { // Fetch Products
                const res = await fetch(`/api/products?page=${page}&limit=9&sort=newest`, { headers });
                if (res.ok) {
                    const data = await res.json();
                ...
                const res = await fetch(`/api/users?page=${page}&limit=10&sort=newest`, { headers });
                if (res.ok) {
                    const data = await res.json();
                    setUsers(data.users);
                    setTotalPages(data.pagination.totalPages);
    ...
    const openProductEditModal = (product) => {
        ...
        setProductFormData({
            ...
            description: product.description, imageUrl: product.imageUrl, inStock: product.inStock
        });
                ...
                <div className="max-w-7xl mx-auto">
                            ...
                            {user.role === "ADMIN" && (
                                <>
                                    ...
                                    <button
                                        ...
                                        className={`px-6 py-2 rounded-full font-medium transition capitalize ${activeTab === "products" ? "bg-primary text-white shadow-md" : "bg-white text-gray-600 hover:bg-gray-100"}`}
                                    >
                                        Products
                                    </button>
                                    <button
                    ...
                    {loading ? (
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden p-6">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {products.map(product => (
                                            <div key={product.id} className="border border-gray-200 rounded-xl p-4 flex flex-col hover:shadow-md transition">
                                                <img src={product.imageUrl || "https://via.placeholder.com/150"} alt={product.name} className="w-full h-40 object-cover rounded-lg mb-4" />
                                                <h3 className="font-bold text-lg text-gray-900">{product.name}</h3>
                                                <p className="text-sm text-gray-500 mb-2">{product.category} • ${product.price}</p>
                                                <span className={`inline-block px-2 py-1 text-xs font-semibold rounded-full mb-4 w-fit ${product.inStock ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                                    {product.inStock ? 'In Stock' : 'Out of Stock'}
                                                </span>
                                                <div className="mt-auto flex justify-end space-x-2">
                                                    <button onClick={() => openProductEditModal(product)} className="text-primary hover:text-primary-dark font-medium text-sm">Edit</button>
                                                    <button onClick={() => handleDeleteProduct(product.id)} className="text-red-500 hover:text-red-700 font-medium text-sm">Delete</button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            {/* Product Modal - Seamless Styling */}
                            {isProductModalOpen && (
                                <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
                                    <div className="absolute inset-0 bg-white/10 backdrop-blur-[2px] pointer-events-auto" onClick={() => setIsProductModalOpen(false)}></div>
                                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto pointer-events-auto border border-gray-200 relative z-10">
                                        <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                                        ...
                                        <form onSubmit={handleProductSubmit} className="p-6 space-y-4">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <input type="text" placeholder="Name" className="border p-2 rounded-lg w-full" value={productFormData.name} onChange={e => setProductFormData({ ...productFormData, name: e.target.value })} required />
                                                <input type="text" placeholder="Category" className="border p-2 rounded-lg w-full" value={productFormData.category} onChange={e => setProductFormData({ ...productFormData, category: e.target.value })} required />
                                                <input type="number" placeholder="Price" className="border p-2 rounded-lg w-full" value={productFormData.price} onChange={e => setProductFormData({ ...productFormData, price: e.target.value })} required />
                                                <input type="text" placeholder="Image URL" className="border p-2 rounded-lg w-full" value={productFormData.imageUrl} onChange={e => setProductFormData({ ...productFormData, imageUrl: e.target.value })} required />
                                            </div>
                                            <textarea placeholder="Description" className="border p-2 rounded-lg w-full h-24" value={productFormData.description} onChange={e => setProductFormData({ ...productFormData, description: e.target.value })} required></textarea>
                                            <div className="flex gap-4">
                                                <label className="flex items-center gap-2">
                                                    <input type="checkbox" checked={productFormData.inStock} onChange={e => setProductFormData({ ...productFormData, inStock: e.target.checked })} /> In Stock
                                                </label>
                                            </div>
                                            <div className="flex justify-end gap-3 pt-4">
                                                <button type="button" onClick={() => setIsProductModalOpen(false)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
                                                <button type="submit" className="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark">{editingProduct ? "Update Product" : "Add Product"}</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
